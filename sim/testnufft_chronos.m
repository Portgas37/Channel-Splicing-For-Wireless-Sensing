...
epsilon = 0.00001; % Adjust epsilon value

p = computeInverseNDFT(cirFiltered, F, alpha, epsilon);

function p = computeInverseNDFT(h, F, alpha, epsilon)
    p = randn(size(F, 2), 1);
    t = 0;

    %problem with the value of gamma : we should use 1/16 but with this
    %value the norm difference diverge to infinity. i tried with this value
    %of gamma and it converges.
    ...
    while ~converged
        ...
        if t > 1000 % Maximum number of iterations
